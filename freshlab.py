...
def main() -> None:
    ...
    ap.add_argument("--hold", type=float, default=None,
                    help="override every family's max hold (min). P3 ships 180; "
                         "the pre-registered families were written at 480.")
    ap.add_argument("--pre-fix", action="store_true",
                    help="reproduce the pre-P1/P2 numbers: fill stops/trails at their "
                         "trigger level and let the entry gate scan unbounded history")
    ...
    # P2 fills and the P1 bounded gate are the defaults; --pre-fix restores the
    # optimistic trigger-price fill and the unbounded cred_peak/nuke lookback.
    fill = ({"use_candle_low": False, "stop_slip_pct": 0.0, "trail_slip_pct": 0.0}
            if args.pre_fix else backtest.FILL_MODEL)
    setup = ({k: v for k, v in backtest.DEFAULT_SETUP.items() if k != "peak_window_min"}
             if args.pre_fix else backtest.DEFAULT_SETUP)
    print(f"engine: fill={'trigger price (PRE-FIX)' if args.pre_fix else 'min(trigger, low)+slip'}"
          f" · gate lookback={'unbounded (PRE-FIX)' if args.pre_fix else str(setup['peak_window_min']) + 'm'}"
          f" · hold={'family default' if args.hold is None else str(args.hold) + 'm'}"
          f" · censored=0.0 (intact_end=False)")
    ...
    def run_sim(tok, c, idx, entry, ex, bucket, out):
        if args.hold is not None:
            ex = {**ex, "hold": args.hold}
        r = sim(c, idx, entry, ex, False, fill=fill)
        if r is not None:
    ...
    for ex_name, ex in (("EA", EA), ("EB", EB)):
        ...
        for tok, c, bucket, idx, entry, pre in base_pop:
            if not backtest.entry_setup_ok(pre, int(c[idx][0]), entry, setup):
                continue
