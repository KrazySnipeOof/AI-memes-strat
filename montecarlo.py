    ...
    if cb and cb.get("raw"):
        ...
        if len(ab) >= 30:
            strategies.append(build_payload(
                ...
                f"biased (curated profitable wallets): it only exists because those wallets were "
                f"picked for past profit. Fills are the corrected model (gap-through stops fill at "
                f"min(trigger, low) + slip). Not proven live - treat as an upper bound.",
...
<main>
  ...
  <div class="card">
    ...
    <ul class="caveats">
      <li><b>How to read this page:</b> the <b>Strategy</b> dropdown swaps between trade pools — every chart, tile and table below re-renders for the selected one. Every pool now runs the corrected engine (censored positions resolved by policy, stops/trails filled below trigger). Under it <b>every strategy the bot actually runs is negative-expectancy</b>, so for those read the scenario table as "how bad, how often" rather than as a survival question. The two wallet-copy pools are the only positives left, and they are not strategies the bot can run — see the next bullet.</li>
      <li><b>The three strategies are not equally trustworthy.</b> <i>Base-entry</i> is the bot's validated backtest sample (the audit below covers it). <i>Copy-all</i> and <i>Cluster-confirmed</i> are sourced from the monitored wallets' real buy stream — unbiased at the token level (rugs included) but still <b>wallet-selection biased</b>: they only exist because those wallets were curated for past profit, so their high win rates partly reflect who was picked, not a repeatable edge. The unbiased test also found cluster-confirm does <b>not</b> beat copy-all. Both now run the corrected fill and censoring model, same as every other pool on this page.</li>
